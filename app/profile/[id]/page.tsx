...
// ── Open or create a conversation then navigate to it ────────────────────────
async function openChat(
  ...
  // Check for existing general conversation (no post)
  const { data: existing } = await supabase
    .from("conversations")
    .select("id")
    .or(
      `and(buyer_id.eq.${currentUserId},seller_id.eq.${sellerId}),` +
      `and(buyer_id.eq.${sellerId},seller_id.eq.${currentUserId})`
    )
    .is("post_id", null)
    .maybeSingle()
...
export default function SellerProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleChat = async () => {
    setChatLoading(true)
    await openChat(currentUserId, sellerId, router)
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .back-btn { background:rgba(255,255,255,0.12); border:none; cursor:pointer; width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; color:white; flex-shrink:0; }
        ...
        .av { width:100px; height:100px; border-radius:50%; border:4px solid white; box-shadow:0 8px 32px rgba(13,29,110,0.35); background:linear-gradient(135deg,#0f1f6e,#f97316); display:flex; align-items:center; justify-content:center; font-size:2rem; font-weight:800; color:white; font-family:'Playfair Display',serif; overflow:hidden; }
        ...
        .chat-btn {
          ...
          background:linear-gradient(135deg,#0f1f6e,#1a2a9a);
          color:white; border:none; cursor:pointer;
          font-family:'DM Sans',sans-serif; font-weight:800; font-size:0.96rem;
          box-shadow:0 6px 20px rgba(13,29,110,0.35);
          transition:all .2s;
        }
        .chat-btn:active { transform:scale(0.98); }
        .chat-btn:disabled { opacity:0.65; cursor:not-allowed; }
        .chat-btn-loading { animation:pulse 1s ease infinite; }

        .own-profile-note { text-align:center; padding:13px; color:#94a3b8; font-size:0.82rem; font-family:'DM Sans',sans-serif; background:#f8fafc; border-radius:12px; border:1.5px dashed #e2e8f0; }
        .login-to-chat { display:flex; align-items:center; justify-content:center; gap:8px; width:100%; padding:14px; border-radius:14px; background:white; color:#0f1f6e; border:2px solid #c7d2fe; cursor:pointer; font-family:'DM Sans',sans-serif; font-weight:700; font-size:0.9rem; text-decoration:none; transition:all .2s; }
      ...
      <div className="body">
        ...
        <div className="stats">
          ...
          <div className="stat-cell">
            ...
            <div className="trust-mini-bar">
              <div className="trust-mini-fill" style={{ width: `${trustScore}%`, background: `linear-gradient(90deg,${trustColor},${trustColor}99)` }} />
            </div>
            <span className="trust-mini-chip" style={{ background: `${trustColor}15`, color: trustColor }}>{trustLabel}</span>
        ...
        <div className="card">
          ...
          {isOwnProfile ? (
            <div className="own-profile-note">This is your profile.</div>
          ) : currentUserId ? (
            <button
              className={`chat-btn ${chatLoading ? "chat-btn-loading" : ""}`}
              onClick={handleChat}
            ...
            <a href="/login" className="login-to-chat">
